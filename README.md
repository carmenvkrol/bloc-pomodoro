...
```
$ git clone https://github.com/carmenvkrol/bloc-pomodoro.git
```
...
The application runs on port 3000 (configured in [/config/env/all.js](https://github.com/carmenvkrol/bloc-pomodoro/blob/master/config/env/all.js)).  To change the port, modify the number highlighted below
```
'use strict';
...
Grunt looks for files using a defined pattern so that it knows what to compile and copy and where to put it. To edit the files that Grunt watches, look at the array of files in the watch task in [Gruntfile.js](https://github.com/carmenvkrol/bloc-pomodoro/blob/master/gruntfile.js). The default watched files are:
...
------------
The client-side files for user authentication, which are used in the [/signup] (https://github.com/carmenvkrol/bloc-pomodoro/blob/master/public/modules/users/views/authentication/signup.client.view.html) and [/signin](https://github.com/carmenvkrol/bloc-pomodoro/blob/master/public/modules/users/views/authentication/signin.client.view.html) views, are in the users directory within the public directory.  The server-side functionality is in the users files within the app folder.  (See Directory Structure section above to locate these).



Timer Feature
------------
The client-side functionality for the timers, which are used in the [/dashboard](https://github.com/carmenvkrol/bloc-pomodoro/blob/master/public/modules/core/views/dashboard.client.view.html) view, can be found in the core directory within the public directory. The server-side functionality is found in the core files within the app folder. (See Directory Structure section above to locate these).
...
**[Method-Override](https://github.com/expressjs/method-override)** - allows the use of HTTP verbs, such as PUT and DELETE, in places where the client doesn't support it
